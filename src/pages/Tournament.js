  ...
  */
	...
	function checkNotStale(gameLog) {
		...
		function checkLine(box) {
			for (let i = 0; i < 3; i++) {
				if (Math.abs(box[i].reduce((acc, val) => acc + val, 0)) === 3)
					return true; // horizontal
				if (
					Math.abs(
						box.map((row) => row[i]).reduce((acc, val) => acc + val, 0)
					) === 3
				)
			...
			if (
				Math.abs(
					box.map((row, i) => row[i]).reduce((acc, val) => acc + val, 0)
				) === 3
			)
				return true;
			if (
				Math.abs(
					box.map((row, i) => row[2 - i]).reduce((acc, val) => acc + val, 0)
				) === 3
			)
				return true;
			return false;
	...
	const run = async (e) => {
		...
		const numberOfGamesToDisplay =
			numberOfGames > games.length ? games.length : numberOfGames;
		const newGameStrings = Array(numberOfGamesToDisplay).fill("");
		let longestGame = games
		...
		<div className="App w-full flex-col justify-center items-center">
			...
			<Button
				...
				onClick={(e) => {
					setColClassName(
						`grid grid-cols-${
							numberOfGames > 4 ? Math.round(numberOfGames / 2) : numberOfGames
						} w-full`
					);
					run(e);
				}}
