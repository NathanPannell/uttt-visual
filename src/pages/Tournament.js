...
function Tournament() {
  ...
  const [numberOfGames, setNumberOfGames] = useState(4);
  const [havePopped, setHavePopped] = useState(false);
  const [roundNumber, setRoundNumber] = useState(1);
  const [colClassName, setColClassName] = useState(
        ...
        matchData.map((match) => {
          ...
          */
          return getWinnerGame(
            match.match_log,
            match.player1 === match.winner ? 1 : 2,
            match.round
          );
        })
      );
    ...
    setMatchData(
      matches.map((match) => {
        return {
          ...
          round: match.round,
          player1: participants.find(
            (participant) => participant.player_id === match.player1_id
          ),
          player2: participants.find(
            (participant) => participant.player_id === match.player2_id
          ),
          winner: participants.find(
            (participant) => participant.player_id === match.winner_id
          ),
  ...
  const getWinnerGame = (match_log, winning_player, round) => {
    let gameString = "";
    let first_player = 0;
    let game_index = 1;
    ...
    for (let i = 0; i < 10; i++) {
      ...
      if (
        (p1_wins && i % 2 !== tempGameString.length % 2) ||
        (!p1_wins && i % 2 === tempGameString.length % 2)
      ) {
        if (
          gameHasWinner(tempGameString) &&
          tempGameString.length > gameString.length
        ) {
  ...
  const run = async (e) => {
    ...
    const numberOfGamesToDisplay =
      numberOfGames > newGames.length ? newGames.length : numberOfGames;
    ...
    for (let i = 0; i < longestGame.length; i++) {
      for (let j = 0; j < numberOfGamesToDisplay; j++) {
        newGameStrings[j] +=
          gamesToDisplay[j].length > i ? gamesToDisplay[j][i] : "";
      }
      setGameStrings([...newGameStrings]);

      if (i > 2) {
        await delay(1000); // Wait for 1 second after playing first 4 moves
      }
    }
  };
  ...
  const runRound = () => {
    ...
    setGames((gamesArr) =>
      gamesArr.sort((game) => (game.round === roundNumber ? -1 : 1))
    );
    setGameData((gamesDataArr) =>
      gamesDataArr.sort((game) => (game.round === roundNumber ? -1 : 1))
    );
    setGameStrings([]);
  };
  ...
  return (
    <div className="App w-full flex-col justify-center items-center mb-96">
      <TextField
        label="Round Number"
        id="round-number-outlined-basic"
        variant="outlined"
        defaultValue={"1"}
        onChange={(e) => {
      ...
      <Button
        ...
        onClick={(e) => {
          setColClassName(
            `grid grid-cols-${
              numberOfGames > 3 ? Math.round(numberOfGames / 2) : numberOfGames
            } w-full`
          );
          run(e);
        }}
      ...
      <Button
        onClick={() => console.log(colClassName)}
        endIcon={<NavigateNextIcon />}
      />
          ...
          if (!game_over) {
            title = (
              <h2 className="text-center mb-2 text-2xl">
                ...
                vs. {gameData[index].player2_name} (
            ...
            title = (
              <h2 className="text-center mb-2 text-2xl">
                <b>
                  {gameData[index].player1_name} (
                  {gameData[index].player1_emoji})
                </b>{" "}
                vs. {gameData[index].player2_name} (
                {gameData[index].player2_emoji})
              </h2>
